...
import { authCookieNames } from "@/services/cognito-auth.server";
import { createDashboardProvider, DashboardApiUnauthorizedError, DashboardApiUnavailableError } from "@/services/dashboard-api.server";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await createDashboardProvider(payload, accessToken);
    ...
    if (error instanceof DashboardApiUnavailableError && [400, 409, 422].includes(error.status)) return NextResponse.json({ code: "provider_creation_rejected" }, { status: error.status, headers });
    return NextResponse.json({ code: "provider_creation_failed" }, { status: 503, headers });
  }
}
